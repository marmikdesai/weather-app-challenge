...
class User extends Component {
  ...
  componentDidMount() {
    ...
    this.props.location.state ?
      this.props.location.state.from.user ? city = this.props.location.state.from.user.city.value : city = this.props.location.state.from.city.value :
      null

    if(this.props.location.state) {
      return fetch('http://api.openweathermap.org/data/2.5/weather?q='+city
+'&appid=359f372936efcf8e93c400c866fdc089&units=metric')
